...
export function parse(str: string, options: ParseOptions = {}): TokenData {
  ...
  function name() {
    ...
    if (ID_START.test(chars[index] as string)) {
      do {
        value += chars[index++];
      } while (ID_CONTINUE.test(chars[index] as string));
    } else if (chars[index] === '"') {
  ...
  while (index < chars.length) {
    const value = chars[index] as string;
  ...
  function consumeUntil(endType: TokenType): Token[] {
    ...
    while (true) {
      const token = tokens[pos++] as LexToken;
      if (token.type === endType) break;

      if (token.type === "char" || token.type === "escape") {
        let path = token.value;
        let cur = tokens[pos] as LexToken;

        while (cur.type === "char" || cur.type === "escape") {
          path += cur.value;
          cur = tokens[++pos] as LexToken;
        }
...
function tokenToFunction(
  ...
  if (token.type === "group") {
    ...
    const result = (data: ParamData) => {
      const [value, ...missing] = fn(data);
      if (!missing.length) return [value];
      return [""];
    };

    return result as (data: ParamData) => string[];
  }
...
export function match<P extends ParamData>(
  ...
  return function match(input: string) {
    ...
    for (let i = 1; i < m.length; i++) {
      ...
      const key = keys[i - 1] as Key;
      const decoder = decoders[i - 1] as Decode;
      params[key.name] = decoder(m[i] as string);
    }
...
function isNameSafe(name: string): boolean {
  ...
  return (
    ID_START.test(first as string) &&
    rest.every((char) => ID_CONTINUE.test(char))
  );
}
...
function isNextNameSafe(token: Token | undefined): boolean {
  if (token && token.type === "text")
    return !ID_CONTINUE.test(token.value[0] as string);
  return true;
}
