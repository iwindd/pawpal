...
import { IPaymentGatewayRepository } from '../../domain/repository.port';

@Injectable()
export class PrismaPaymentGatewayRepository implements IPaymentGatewayRepository {
  ...
  async updatePromptpayManualMetadata(metadata: PromptpayManualInput) {
    ...
    return this.prisma.paymentGateway.update({
      where: { id: gateway.id },
      data: { metadata },
      select: { id: true, name: true, label: true, metadata: true },
    });
  }
}
